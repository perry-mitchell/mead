...
ipcMain.on("editorReady", function(event, info) {
  const { windowID } = info;
  const { window, filePath } = windows[windowID];
  if (filePath) {
...
ipcMain.on("save", function(event, info) {
  const { contents, windowID } = info;
  const { filePath, window } = windows[windowID];
  if (filePath) {
    fs.writeFileSync(filePath, contents, "utf8");
    windows[windowID].dirty = false;
    window.webContents.send("setDirty", { dirty: false });
  } else {
    dialog.showSaveDialog(window, { title: "Save new markdown file" }, function(filename) {
      if (filename) {
...
ipcMain.on("edit", function(event, info) {
  const { contents, windowID } = info;
  // console.log("EDIT", contents, windowID);
  checkDirty(windowID, contents);
  // const { dirty, window } = windows[windowID];
  // if (!dirty) {
  //   windows[windowID].dirty = true;
  //   window.webContents.send("setDirty", { dirty: true });
  // }
  // let { dirtyCheck } = windows[windowID];
  // if (filePath && !dirty) {
  //   // debounce(dirtyCheck, 5000);
  //   dirtyCheck = dirtyCheck || debounce(() => {
  //     readFile(filePath).then(readContents => {
...
function checkDirty(windowID, contents) {
  const { dirty, filePath, window } = windows[windowID];
  if (filePath) {
    const dirtyCheck = windows[windowID].dirtyCheck || debounce(() => {
      readFile(filePath, "utf8").then(readContents => {
        ...
        if (isNowDirty !== dirty) {
          ...
          window.webContents.send("setDirty", { dirty: isNowDirty });
        }
      });
    }, 1000);
    dirtyCheck();
...
function createWindow(filePath = null) {
  ...
  // if (filePath !== null) {
    ...
    // win.webContents.on('editorReady', () => {
    //   console.log("READY");
    //   readFile(localPath, "utf8")
    //     .then(contents => {
    //       console.log("SEND", contents);
    //       win.webContents.send('setContents', { contents });
    //     });
    // });
  // }
